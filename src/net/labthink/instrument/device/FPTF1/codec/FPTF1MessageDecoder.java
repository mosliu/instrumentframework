...
/**
 * FPTF1's MessageDecoder
 *
 * @version        1.0.0.0, Aug 24, 2010
 * @author         Moses
 */
public class FPTF1MessageDecoder extends AbstractPacketDecoder {

    public FPTF1MessageDecoder() {
	super(FPTF1InMessageParams.PACKET_HEADER, FPTF1InMessageParams.PACKET_TAIL);// 添加包头
    }

    @Override
    protected AbstractPacket decodeBody(IoSession session, IoBuffer in) {
	int length = FPTF1InMessageParams.PACKET_FULL_LEN-FPTF1InMessageParams.PACKET_HEADER_LEN;
	...
	FPTF1InMessage m = new FPTF1InMessage();
	byte[] content = new byte[FPTF1InMessageParams.PACKET_BODY_LEN];
	short tail ;
	in.get(content);
...
//	if (in.remaining() < FPTF1InMessageParams.PACKET_HEADER_LEN) {
...
//	}
        ...
        if (header == _header&&tail == in.getShort(FPTF1InMessageParams.PACKET_FULL_LEN-FPTF1InMessageParams.PACKET_TAIL_LEN)) {
    ...
    public MessageDecoderResult decode(IoSession session, IoBuffer in,
        ...
        pkt.setTail(tail);
        pkt.setLength(FPTF1InMessageParams.PACKET_FULL_LEN);
        out.write(pkt);
